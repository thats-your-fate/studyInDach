...
export const metadata: Metadata = {
	title: "Orientación gratuita para estudiar | Study in DACH",
	description: "Contacta con Study in DACH para orientación gratuita sobre programas en Alemania, Austria y Suiza.",
	robots: {
		index: false,
		follow: true,
	},
	alternates: {
		canonical: absoluteUrl("/es/contacto"),
	},
}
...
async function submitInquiry(formData: FormData) {
	...
	const program = programId
		? await prisma.degreeProgram.findUnique({
			...
			include: {
				university: true,
				translations: { where: { locale: "es" }, take: 1 },
			},
		})
		: null
	...
	await prisma.inquiry.create({
		data: {
			...
			programNameSnapshot: program ? program.translations[0]?.localizedProgramName || program.programName : null,
...
function contactReturnPath(programId: number | null) {
	return programId ? `/es/contacto?programId=${programId}` : "/es/contacto"
}
...
function contactStatePath(programId: number | null, key: "sent" | "error", value: string) {
	...
	return `/es/contacto?${params.toString()}`
}
