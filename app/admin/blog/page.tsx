...
async function createBlogPost(formData: FormData) {
	...
	const post = await prisma.blogPost.create({
		data: {
			...
			authorName: nullableField(formData, "authorName") || "Study in DACH",
...
async function upsertTranslation(formData: FormData) {
	...
	redirect(`/admin/blog?post=${postId}`)
}
...
export default async function AdminBlogPage({ searchParams }: { searchParams?: BlogAdminSearchParams }) {
...
function PostEditor({ post, categories, tags }: { post: any; categories: any[]; tags: any[] }) {
...
function BlogCreateForm({ categories, tags }: { categories: any[]; tags: any[] }) {
	return (
		<form action={createBlogPost} className="row g-3">
			...
			<TextInput label="Author" name="authorName" value="Study in DACH" className="col-md-6" />
...
function PostList({ posts }: { posts: any[] }) {
	if (!posts.length) {
		return <div className="empty-results"><h5>No blog posts yet</h5><p>Create the first post from the form on the left.</p></div>
	}
	return (
...
function Panel({ eyebrow, title, actions, children }: { eyebrow: string; title: string; actions?: ReactNode; children: ReactNode }) {
	return (
