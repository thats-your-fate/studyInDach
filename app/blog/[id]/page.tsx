...
export async function generateMetadata({ params }: BlogDetailsProps): Promise<Metadata> {
	const post = await getPublishedBlogPost(params.id, "en")
	if (!post) return {}
	...
	return {
		...
		alternates: {
			canonical: absoluteUrl(localizedBlogPostUrl(post.slug, "en")),
		...
		openGraph: {
			...
			url: absoluteUrl(localizedBlogPostUrl(post.slug, "en")),
...
export default function BlogDetails({ params }: BlogDetailsProps) {
	return <BlogDetailPage slug={params.id} locale="en" backLabel="Back to blog" />
}
